...
Uploaded HTML/CSS/JS কখনো trusted `www/` DOM-এ inject বা parent-এর আসল NativeKit-এ attach হয় না। Native path private staged package, per-app virtual origin/profile, Android separate app process + authenticated bounded IPC এবং iOS isolated controller/watchdog ব্যবহার করে। Installed façade hard-coded method allowlist, native-held app/session identity, app/capability/method policy, rate/payload quota এবং bounded metadata audit-এ সীমিত; raw Capacitor plugin নেই। Permitted fallback-এ `sandbox="allow-scripts"` opaque-origin iframe চলে এবং maximum process isolation থাকে না।
