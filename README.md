...
- Node **22.23.0**-এ সর্বশেষ `npm run check` pass: config validation, strict typecheck, **75/75 Vitest (7 files)**, bridge/type generation এবং native staging সফল। Direct tests exact installed result/event sanitization, camera/filesystem bounds, hidden API, quota rejection এবং package-preserving `usage/cleanup` success/failure-ও যাচাই করে। Node 22-এর read-only global `navigator`-এর সঙ্গেও focused HTTP/stream test shim compatible। Project engine/CI requirement **Node 22+**। সর্বশেষ full lockfile ও production-only `npm audit --audit-level=low`—উভয়টিতে **0 vulnerability**; `npm ls --all`-ও clean।
